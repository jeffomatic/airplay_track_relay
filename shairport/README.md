...
```
./configure --with-rtsp-relay
```

Have a UDP listener on port 12345. You'll get messages like this:

```
header count (4 - int32)
header names
  each header:
    header name byte count (4 - int32)
    header name bytes (?)
  ...
header values
  each header:
    header value byte count (4 - int32)
    header name bytes (?)
content byte count (4 - int32)
content value bytes (?)
method bytes (16)
